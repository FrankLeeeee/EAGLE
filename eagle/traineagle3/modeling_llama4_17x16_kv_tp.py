...
class Llama4TextExperts(nn.Module):
    def __init__(self, config: Llama4TextConfig):
        ...
        self.expert_dim_per_shard = self.expert_dim // dist.get_world_size(self.tp_group)

        self.gate_up_proj = nn.Parameter(torch.empty(self.num_experts, self.hidden_size, 2 * self.expert_dim_per_shard))
        self.down_proj = nn.Parameter(torch.empty((self.num_experts, self.expert_dim_per_shard, self.hidden_size)))
...
class Llama4ForCausalLM(Llama4PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        logit_list = [torch.empty_like(logits) for _ in range(dist.get_world_size(self.tp_group))]
